import { MessageSquare, Users, FileText, Sparkles, Send, MicOff, Mic, VideoOff, Video as VideoIcon, Check, X, Captions } from 'lucide-react';
    ...
    <div className={`h-full w-full md:w-[360px] shrink-0 bg-[#111114] border-l border-white/[0.06] flex flex-col shadow-2xl transition-all duration-300 ease-in-out z-40 ${
      ...
      <div className="flex p-2 border-b border-white/[0.06] gap-1 shrink-0">
        {[
          ...
          { key: 'transcript', icon: <Captions className="h-3.5 w-3.5" />, label: 'Transcript' },
          { key: 'participants', icon: <Users className="h-3.5 w-3.5" />, label: `People (${totalParticipants})` },
          { key: 'summary', icon: <FileText className="h-3.5 w-3.5" />, label: 'Summary' },
        ].map(tab => (
          <button
            key={tab.key}
      ...
      {activeTab === 'transcript' && (
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="p-3 border-b border-white/[0.06] shrink-0 bg-white/[0.02]">
            <p className="text-xs text-white/50">Live transcript generated by speech recognition. (Only visible to you if they are your captions, or to everyone if broadcasted via the server).</p>
          </div>
          <div className="flex-1 p-3 overflow-y-auto space-y-3 custom-scrollbar flex flex-col">
            {(!meetingTranscript || meetingTranscript.length === 0) && (
              <div className="flex-1 flex flex-col items-center justify-center text-white/20 gap-3">
                ...
                <p className="text-[10px] text-center max-w-[200px]">The host must turn on Captions to start recording speech.</p>
              </div>
            )}
            {meetingTranscript?.map((msg, idx) => (
              <div key={idx} className="flex flex-col items-start mb-2">
                <div className="flex items-center gap-1.5 mb-1 px-1">
                  <span className="text-[11px] font-medium text-emerald-400">{msg.senderName}</span>
                  <span className="text-[10px] text-white/20">
                    {new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
                <div className="px-3 py-2 rounded-xl text-sm leading-relaxed bg-white/[0.03] text-white/90 border border-white/[0.06] w-full">
                  {msg.text}
                </div>
              </div>
            ))}
      ...
      {activeTab === 'chat' && (
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="p-3 border-t border-white/[0.06] bg-[#111114]">
            <div className="text-[10px] text-white/40 mb-2 flex items-center gap-1.5 px-1">
              <Sparkles className="h-3 w-3 text-primary" />
              <span>Hint: Start a message with <strong className="text-white/70">@IntellBot</strong> to ask the AI assistant.</span>
            </div>
